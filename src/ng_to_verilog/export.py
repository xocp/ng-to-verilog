...
def export(topLevelComponentName, ngData):
	...
	with open(buildFilename, "w") as build_file:
		buildScriptTemplate \
			.stream({ "topLevelComponentName" : testbenchComponentName, "exportedComponents" : _exported_components}) \
			.dump(build_file)

	print(f"Export complete. Proceed with editing the testbench file ({ngv.get_output_folder()}/{testbenchComponentName}.v), and then execute \"{buildFilename}\" in the output directory to build, run and view the simulation results.\n")
...
def export_module(name_or_key, ngData, env, node=None):
	...
	if component["name"] in _exported_components or \
		component["name"] in ['CONST']:
		return component
	else:
